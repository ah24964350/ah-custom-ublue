...
### LY DISPLAY MANAGER (built from source with Zig)
# Exact dependencies recommended by the official ly project for Fedora
dnf5 install -y --setopt=install_weak_deps=0 \
    zig \
    pam-devel \
    libxcb-devel \
...
# Clone → build → install (uses the official "installexe" target for systemd)
git clone --depth 1 https://codeberg.org/fairyglade/ly.git /tmp/ly
cd /tmp/ly
zig build installexe -Dinit_system=systemd
cd /
...
# Make ly the default on tty2 (disables any other DMs + getty)
systemctl disable gdm.service sddm.service lightdm.service || true
systemctl disable getty@tty2.service || true
systemctl enable ly@tty2.service
...
### Install packages
dnf5 install -y adw-gtk3-theme cmatrix fastfetch geany gtkhash-thunar \
    qt5-qtwayland qt5ct qt6-qtwayland qt6-qtwayland-adwaita-decoration qt6ct \
    sway-config-fedora Thunar-docs thunar-media-tags-plugin thunar-vcs-plugin thunar-volman vlc 
...
# Build dependencies + runtime deps (exact list from official ly docs)
dnf5 install -y --setopt=install_weak_deps=0 \
    zig \
    pam-devel \
    libxcb-devel
...
# Clone → build → install (official systemd method)
git clone --depth 1 https://codeberg.org/fairyglade/ly.git /tmp/ly
cd /tmp/ly
zig build installexe -Dinit_system=systemd
cd /
...
### === LY DISPLAY MANAGER (built from source) ===
# Build dependencies (official from ly README)
dnf5 install -y --setopt=install_weak_deps=0 \
    zig \
    pam-devel \
    libxcb-devel
...
# Clone, build and install ly (official systemd method)
git clone --depth 1 https://codeberg.org/fairyglade/ly.git /tmp/ly
cd /tmp/ly
zig build installexe -Dinit_system=systemd
cd /
